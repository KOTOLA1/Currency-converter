...
// List of currencies and their country codes for flags
const currencies = {
    "USD": "US",
    "ETB": "ET",
    "EUR": "EU",
...
function populateDropdowns() {
    for (let code in currencies) {
        ...
        let option2 = `<option value="${code}" ${code === "ETB" ? "selected" : ""}>${code}</option>`;
...
function updateFlag(element) {
    ...
    const countryCode = currencies[currencyCode];
...
async function getExchangeRate() {
    ...
    try {
        ...
        btnText.innerText = "Get Exchange Rate";
    }
}
